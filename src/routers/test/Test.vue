        ...
        methods: {
            ...
            getHistoryData (selectedDate) {
                ...
                getHistoryData(selectedDate).then((res) => {
                    ...
                    t_data.sportStepCount = data.sportMap.runNum;
                    ...
                    if(typeof(data.hrMap)=='object'){
                        ...
                        hrX.map((item,index)=>{
                            ...
                            t_data.dataHandSkinTemp.push({name: item,value: [selectedDate+' '+item, data.hrMap.surfaceTemList[index]]});
                        })
                    ...
                    if(typeof(data.enviMap)=='object'){
                        ...
                        console.error('腕部温度数据')
                        console.error(t_data.dataWristTemperature)
                    }
            ...
            * */
            ...
            chartHeartRate(anchor) {
                var t_data = this;
                t_data.chartSingleLine('heartRate',anchor,'次/分钟',[0, '100%'],'#ff1432',t_data.dataHeartRate);
            },
            //绘制手表皮温度图表
            chartHandSkinTemp(anchor) {
                var t_data = this;
                t_data.chartSingleLine('hand',anchor,'℃',[0, '100%'],'#59c2ff',t_data.dataHandSkinTemp);
            ...
            chartWristHumidity(anchor) {
                ...
                t_data.chartDoubleLine(elID,anchor,yAxisNameArr,boundaryGapArr,seriesNameArr,seriesColorArr,seriesDataArr);
            },
            //绘制气压图表
            chartPressure(anchor) {
                var t_data = this;
                t_data.chartSingleLine('altitude',anchor,'mb',[0, '100%'],'#006934',t_data.dataPressure);
            },
            ...
            * */
            //绘制单线图表
            chartSingleLine(elID,anchorDateArr,yAxisName,boundaryGap,seriesColor,seriesData) {
                ...
                tooltip: {
                    ...
                    formatter: function (params) {
                        if(elID=='hand'){
                            console.error('我是温度要显示的内容')
                            console.error(params)
                            console.log(`显示的值：${params.name + ' : ' + params.value[1]}`)
                        }
                        params = params[0];
            ...
            //绘制双线图表
            chartDoubleLine(elID,anchorDateArr,yAxisNameArr,boundaryGapArr,seriesNameArr,seriesColorArr,seriesDataArr){
                //如果dom上已经存在画图实例，则只需要从DOM上获取实例方法，如果不存在画图实例，就初始化实例
                var myChart =  echarts.getInstanceByDom(document.getElementById(elID)) || echarts.init(document.getElementById(elID));
                var anchor = anchorDateArr;
